...
This implies that the strategy Majority requires knowledge of all previous states, and therefore could not be represented as a FSM.
However in Theorem \ref{thm:fsm} it is stated that if the number of turns in a game is known, any strategy can be represented as an FSM.
This will be proved in two different ways:
\begin{itemize}
  \item Firstly, by extending the definition of an
  \item If the opponent has defected the majority of the time, Majority will defect
\end{itemize}
